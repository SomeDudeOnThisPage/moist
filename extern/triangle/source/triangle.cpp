...
double *triangle_reference_sample ( int n )
...
//  Input:
//
//    int N, the number of points to sample.
//
//  Output:
//
//    double TRIANGLE_REFERENCE_SAMPLE[2*N], a points in the triangle.
//
{
  double alpha;
  double beta;
...
double *triangle_sample ( double t[2*3], int n )
...
//  Output:
//
//    double TRIANGLE_SAMPLE[2*N], random points in the triangle.
//
{
  double alpha;
  double beta;
...
double *triangle_sample_reflect ( double t[2*3], int n )

//****************************************************************************80
//
//  Purpose:
//
//    triangle_sample_reflect() returns random points in a triangle by reflection
//
//  Licensing:
//
//    This code is distributed under the MIT license.
...
//  Output:
//
//    double triangle_sample_reflect[2*N], random points in the triangle.
//
{
  int i;
  int j;
  ...
  for ( j = 0; j < n; j++ )
  {
    p[0+j*2] = ( 1.0 - u[j] - v[j] ) * t[0+0*2]
                     + u[j]          * t[0+1*2]
                            + v[j]   * t[0+2*2];

    p[1+j*2] = ( 1.0 - u[j] - v[j] ) * t[1+0*2]
                     + u[j]          * t[1+1*2]
                            + v[j]   * t[1+2*2];
  }
