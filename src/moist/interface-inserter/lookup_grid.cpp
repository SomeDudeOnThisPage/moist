...
void moist::LookupGridExact::Initialize(const moist::ExactMesh& mesh, const double resolution)
{
    _mesh = &mesh;
    _grid.clear();
    ...
    for (const auto& point : _mesh->Points())
    {
        _min_bounds.x = std::min(_min_bounds.x, point.x());
        _min_bounds.y = std::min(_min_bounds.y, point.y());
        _max_bounds.x = std::max(_max_bounds.x, point.x());
        _max_bounds.y = std::max(_max_bounds.y, point.y());
    }

    _cell_size = (_max_bounds - _min_bounds) / _resolution;

    for (std::size_t c = 0; c < _mesh->Cells().size(); c++)
    {
        this->InsertCell(c);
    }
}

void moist::LookupGridExact::InsertCell(const std::size_t c)
{
    auto covered_cells = this->GetCells(moist::create_interface_cell_bbox2d_exact(c, *_mesh));
...
std::vector<moist::LookupGridExact::GridCell> moist::LookupGridExact::GetCells(const geo::Box2d& aabb) const
{
    ...
    auto to_cell = [&](const moist::ExactMesh::ExactPoint& p)
    {
        int ix = std::clamp(static_cast<int>((p.x() - _min_bounds.x) / _cell_size.x), 0, static_cast<int>(_resolution) - 1);
        int iy = std::clamp(static_cast<int>((p.y() - _min_bounds.y) / _cell_size.y), 0, static_cast<int>(_resolution) - 1);
        return std::make_pair(ix, iy);
    };

    const GridCell min_cell
    {
        std::clamp(static_cast<int>((aabb.xy_min[0] - _min_bounds.x) / _cell_size.x), 0, static_cast<int>(_resolution) - 1),
        std::clamp(static_cast<int>((aabb.xy_min[1] - _min_bounds.y) / _cell_size.y), 0, static_cast<int>(_resolution) - 1)
    };

    const GridCell max_cell
    {
        std::clamp(static_cast<int>((aabb.xy_max[0] - _min_bounds.x) / _cell_size.x), 0, static_cast<int>(_resolution) - 1),
        std::clamp(static_cast<int>((aabb.xy_max[1] - _min_bounds.y) / _cell_size.y), 0, static_cast<int>(_resolution) - 1)
    };
